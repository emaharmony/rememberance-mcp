...
SQLite is the source of truth. The brain repo at ~/.recall/brain/
...
from recall_mcp.compat import resolve_home
...
class MarkdownSync:
    ...
    Usage:
        sync = MarkdownSync(entity_store, brain_dir=Path("~/.recall/brain"))
    ...
    def __init__(self, entity_store: EntityStore, brain_dir: Optional[Path] = None):
        ...
        self.brain_dir = brain_dir or resolve_home() / "brain"
        ...
        for md_file in entities_dir.rglob("*.md"):
            ...
            if compiled_truth and compiled_truth != entity.get("compiled_truth", ""):
                self.entity_store.update_entity(
                    entity_id, compiled_truth=compiled_truth
                )
                # Add timeline entry about manual edit
                time.strftime("%Y-%m-%d", time.localtime(file_mtime))
    ...
    def _build_page(self, entity: dict) -> str:
        ...
        compiled_truth = (
            entity.get("compiled_truth", "") or f"{entity['name']} — {entity['type']}"
        )
