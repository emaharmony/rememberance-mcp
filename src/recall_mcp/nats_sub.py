...
Recall subscribes to the NATS bus (same bus Prism uses) and
automatically captures agent output events. This means:

1. Prism doesn't need to call Recall synchronously for capture.
2. Recall ingests events as they happen — no data loss.
...
- Each capture uses (agent_id + session_id + turn_number) as an
  ...
  network hiccup), Recall skips the duplicate.
...
class NatsSubscriber:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run, daemon=True, name="recall-nats"
        )
        self._thread.start()
        logger.info(f"Recall NATS subscriber started on {self.nats_url}")
    ...
    def stop(self) -> None:
        ...
        logger.info("Recall NATS subscriber stopped")
...
# The durable implementation is the public 2.1 subscriber. The legacy class
# remains above solely so older imports and source references keep working.
from recall_mcp.nats_jetstream import (  # noqa: E402,F811
    DurableNatsSubscriber as NatsSubscriber,  # noqa: F401
)
