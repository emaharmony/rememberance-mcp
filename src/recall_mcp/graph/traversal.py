    ...
    def get_context(self, entity_id: str, depth: int = 1) -> str:
        ...
        for entity in result["entities"]:
            if entity["id"] == entity_id:
                continue
            edge_desc = self._describe_relationship(
                entity_id, entity["id"], result["edges"]
            )
            truth = entity.get("compiled_truth", "")
            truth_line = f" — {truth}" if truth else ""
            lines.append(
                f"- {entity['name']} ({entity['type']}) {edge_desc}{truth_line}"
            )

        return "\n".join(lines)

    def _describe_relationship(
        self, from_id: str, to_id: str, edges: list[dict]
    ) -> str:
