...
# Absolute package import, not a relative one: this module is invoked both as
# `python -m recall_mcp.integrations.claude_code.capture_transcript` and as a
# bare script (`python .../capture_transcript.py`), which is how the Claude
# Code hook runner calls it. Direct-script execution loads this file as
# `__main__` with no package context, so a relative import would fail.
# Absolute imports work in both modes because recall_mcp is installed
# (editable or wheel) in the interpreter the hook invokes.
...
def _spawn_worker(payload_path: str) -> None:
    ...
    out: int | IO[str] = subprocess.DEVNULL
    if os.environ.get("REM_DEBUG"):
        try:
