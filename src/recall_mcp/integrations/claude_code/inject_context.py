...
context delivery is implemented.
...
Env:
  ...
  RECALL_TIMEOUT       base per-request timeout in seconds (default 6); the
                        task-bootstrap call is capped at min(this, 3s) and the
                        delivery call at min(this, 5s) so the combined hook
                        stays well under Claude Code's hook timeout
...
# `python -m recall_mcp.integrations.claude_code.inject_context` and as a
# bare script (`python .../inject_context.py`, exactly how the Claude Code
# hook runner calls it). Direct-script execution loads this file as
# `__main__` with no package context, so `from . import recall_client` would
# raise "attempted relative import with no known parent package". Absolute
# imports work in both invocation modes as long as recall_mcp itself is
# installed (editable or wheel) in the interpreter that runs the hook --
# which it always is, since the hook command invokes that same venv's
# python.exe.
...
BOOTSTRAP_TIMEOUT = min(TIMEOUT, 3.0) if TIMEOUT > 0 else 3.0
DELIVER_TIMEOUT = min(TIMEOUT, 5.0) if TIMEOUT > 0 else 5.0
LIMIT = int(get_env("INJECT_LIMIT", "8"))
MAX_TOKENS = get_env("MAX_TOKENS")

# Passed to recall_client.derive_scope()/new_client_state() and used to
# namespace this adapter's on-disk CAG client-state file (see
# recall_client._state_path()). Distinct from the agent-neutral identity
# ensure_task() uses for the shared per-repository continuity task -- see
...
def _cag_context(cwd: str, session_id: str) -> str | None:
    ...
    idempotency_key = f"claude-code:deliver:{session_id}" if session_id else None
    try:
