...
# `python -m recall_mcp.integrations.codex.capture_turn` and as a bare script
# (`python .../capture_turn.py`), which is how Codex's hook runner calls it.
# Direct-script execution loads this file as `__main__` with no package
# context, so a relative import would fail. Absolute imports work in both
# modes because recall_mcp is installed (editable or wheel) in the
