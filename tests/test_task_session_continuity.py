...
def test_current_migration_is_fresh_and_idempotent(tmp_path):
    ...
    assert first.to_version == CURRENT_SCHEMA_VERSION == 10
    assert repeated.from_version == repeated.to_version == 10
...
def test_current_migration_upgrades_every_supported_baseline(tmp_path, baseline):
    ...
    assert upgraded.to_version == 10
    assert [item.version for item in upgraded.applied] == list(range(baseline + 1, 11))
