"""Chunking primitive tests (`chunk_text`, semantic-retrieval.md §5.4).

Deterministic pure-function tests: single-chunk collapse, overlap, full
coverage, and configurability via args + env.
"""

from recall_mcp.chunking import CHARS_PER_TOKEN, chunk_text
...
def test_long_text_splits_into_multiple_chunks():
    ...
    text = _make_words(60)
    chunks = chunk_text(
        text, target_tokens=10, overlap_tokens=2, single_chunk_threshold_tokens=10
    )
    assert len(chunks) > 1
...
def test_chunks_cover_all_words_in_order():
    text = _make_words(60)
    chunks = chunk_text(
        text, target_tokens=10, overlap_tokens=3, single_chunk_threshold_tokens=10
    )
...
def test_smaller_target_yields_more_chunks():
    ...
    few = chunk_text(
        text, target_tokens=20, overlap_tokens=2, single_chunk_threshold_tokens=20
    )
    many = chunk_text(
        text, target_tokens=5, overlap_tokens=1, single_chunk_threshold_tokens=5
    )
...
def test_overlap_clamped_below_target_still_progresses():
    ...
    text = _make_words(40)
    chunks = chunk_text(
        text, target_tokens=5, overlap_tokens=999, single_chunk_threshold_tokens=5
    )
    assert len(chunks) >= 1
...
def test_explicit_args_win_over_env(monkeypatch):
    monkeypatch.setenv("RECALL_CHUNK_SINGLE_THRESHOLD_TOKENS", "1")
    text = _make_words(60)
...
def test_legacy_remembrance_env_prefix_is_honored(monkeypatch):
    # recall_mcp.compat.get_env falls back to REMEMBRANCE_* for every RECALL_*
    # variable; chunking must honor that like the rest of the codebase.
    monkeypatch.delenv("RECALL_CHUNK_SINGLE_THRESHOLD_TOKENS", raising=False)
